...
import nl.ttva66.client.output.Generator;
import nl.ttva66.client.output.PDF;
...
public class SendAllMail extends JFrame implements ActionListener {
	...
	public SendAllMail(DagRequest request, MainWindow parent) {
		...
		testmode.setText((Boolean.parseBoolean(API.get("testmode")) ? "Ja"
				: "Nee"));
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				for (int i = 0; i < users.size(); i++) {
					...
					if (email.isEmpty()) {
						throw new RuntimeException(
								"Email adres voor gebruiker " + naam
										+ " is leeg.");
					}
